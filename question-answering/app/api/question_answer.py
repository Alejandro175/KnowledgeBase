...
from app.db.schema import driver
...
def get_llm(api_key: str = config.openai_api_key, model: str = config.answer_llm) -> OpenAILLM:
...
def process_request_context(request: LLMRequest, service: LLMResponseService = Depends(get_qa_service)):
    ...
    """
    print(f"DEBUG Request RAG: {request}")
    try:
        print(request)
        answer, cypher_query, context = service.contextual_answer(question=request.question)
        print(answer)
        return AnswerWithContext(answer=answer, query=cypher_query, context=context)
...
def process_request_context(request: LLMRequest, service: LLMResponseService = Depends(get_qa_service)):
    ...
    """
    print(f"DEBUG Request LLM: {request}")
    try:
